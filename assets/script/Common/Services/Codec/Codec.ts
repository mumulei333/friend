import { AbstractCodec } from "../../../Framework/Defineds/Interfaces/IMessage";
...
export class Codec extends AbstractCodec {
    ...
    pack(data: MessageStruct): boolean {
        ...
        dataView.setUint32(offset, this.mainCmd, Config.USING_LITTLE_ENDIAN);
        offset += Uint32Array.BYTES_PER_ELEMENT;
        dataView.setUint32(offset, this.subCmd, Config.USING_LITTLE_ENDIAN);
        offset += Uint32Array.BYTES_PER_ELEMENT;
        dataView.setUint32(offset, this._dataSize, Config.USING_LITTLE_ENDIAN);
        offset += Uint32Array.BYTES_PER_ELEMENT;
        if (data.buffer) {
            //感觉这里的复制数据有点low啊
    ...
    unPack(data: any): boolean {
        ...
        //取包头
        let offset = 0;
        this.mainCmd = dataView.getUint32(offset, Config.USING_LITTLE_ENDIAN);
        offset += Uint32Array.BYTES_PER_ELEMENT;
        this.subCmd = dataView.getUint32(offset, Config.USING_LITTLE_ENDIAN);
        offset += Uint32Array.BYTES_PER_ELEMENT;
        this._dataSize = dataView.getUint32(offset, Config.USING_LITTLE_ENDIAN);
        offset += Uint32Array.BYTES_PER_ELEMENT;
