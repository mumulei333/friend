...
type JsonMessageConstructor = typeof JsonSerialize;
 ...
 */
export abstract class JsonSerialize extends AbstractSerialize {
    private _data = null;
    ...
    protected serialize(): any {
        ...
        for (let len = serializeKeyList.length, i = 0; i < len; i++) {
            ...
            let serializeObj = this._serializeMember(this[memberName]);
     ...
     * @param value 该成员变量的值
     * */
    private _serializeMember(value: any) {
        if (typeof value == 'number') {
            ...
            return this._serializeMap(value);
    ...
    private _serializeObject(value: Object) { return value }

    private _serializeNumber(value: Number) {
    ...
    private _serializeArray(value: Array<any>) {
        let result = [];
        value.forEach(element => {
            result.push(this._serializeMember(element));
        });
        return result;
    }

    private _serializeMap(value: Map<any, any>) {
        let result = [];
        let self = this;
        value.forEach((value, key) => {
            let serVal = { k: self._serializeMember(key), v: self._serializeMember(value) };
            if (null === serVal.k) {
    ...
    Deserialize(data: Uint8Array): boolean {
        if (data) {
            ...
            return this._deserialize(this._data);
        }
        return false;
    }
    ...
    private _deserialize(data: any) {
        ...
        for (let len = serializeKeyList.length, i = 0; i < len; i++) {
            ...
            let iscomplete = this._deserializeMember(memberName, memberType, arrOrmapKeyType, mapValType, data[serializeKey]);
            if (!iscomplete) {
     ...
     */
    private _deserializeMember(memberName: any, memberType: any, arrOrmapKeyType: any, mapValType: any, value: any) {
        try {
            let originValue = this[memberName];
            if (typeof originValue === 'number') {
                this[memberName] = this._deserializeNumber(memberName, value);
            } else if (typeof originValue === 'string') {
                this[memberName] = this._deserializeString(memberName, value);
            } else if (originValue instanceof Array) {
                this._deserializeArray(memberName, arrOrmapKeyType, value);
            } else if (originValue instanceof Map) {
                this._deserializeMap(memberName, arrOrmapKeyType, mapValType, value);
            } else if (originValue instanceof JsonSerialize) {
                originValue._deserialize(value);
            } else if (null === originValue) {
                switch (memberType) {
                    case Number: this[memberName] = this._deserializeNumber(memberName, value); break;
                    case String: this[memberName] = this._deserializeString(memberName, value); break;
                    case Array: this[memberName] = []; break;
                    case Map: this[memberName] = new Map; break;
                    default: {
                        this[memberName] = new memberType;
                        if (this[memberName] instanceof JsonSerialize) {
    ...
    private _deserializeNumber(memberName: any, value: any) {
    ...
    private _deserializeString(memberName: any, value: any) {
    ...
    private _deserializeArray(memberName: any, valueType: any, value: any) {
        ...
        value.forEach((element, i) => {
            if (valueType === Number) {
                this[memberName].push(this._deserializeNumber(memberName + "[" + i + "]", element));
            } else if (valueType === String) {
                this[memberName].push(this._deserializeString(memberName + "[" + i + "]", element));
            ...
            } else if (this[memberName] instanceof JsonSerialize) {
                this[memberName].deserialize(element);
            } else {
                let elementObj = new valueType;
                if (elementObj instanceof JsonSerialize) {
                    elementObj._deserialize(element);
    ...
    private _deserializeMap(memberName: any, keyType: any, valueType: any, value: any) {
        if (!(value instanceof Array)) {
        ...
        value.forEach((element, i) => {
            ...
            let elementKey;
            if (keyType === Number) {
                elementKey = this._deserializeNumber(memberName + "[" + i + "]:key", element.k);
            } else if (keyType === String) {
                elementKey = this._deserializeString(memberName + "[" + i + "]:key", element.k);
            } else {
            ...
            let elementValue;
            if (valueType === Number) {
                elementValue = this._deserializeNumber(memberName + "[" + i + "]:value", element.v);
            } else if (valueType === String) {
                elementValue = this._deserializeString(memberName + "[" + i + "]:value", element.v);
            } else if (valueType === Array) {
                ...
                if (elementValue instanceof JsonSerialize) {
                    elementValue._deserialize(element.v);
                } else {
