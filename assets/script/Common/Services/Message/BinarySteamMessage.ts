...
import { StringToUtf8Array } from "../../Utils/StringToUtf8Array";
import { Utf8ArrayToString } from "../../Utils/Utf8ArrayToString";

type BinaryStreamConstructor = typeof BinarySerialize;
...
class StreamValue<T> implements IStreamValue {
    ...
    get littleEndian() {
        return Config.USING_LITTLE_ENDIAN;
    }
}
...
export abstract class BinarySerialize extends AbstractSerialize {
    ...
    /**@description 是否是数值类型 */
    private _isNumberValue(valueType: any) {
    ...
    private _isStringValue(valueType: any) {
    ...
    protected memberSize(value: any, valueType: any, arrTypeOrMapKeyType: any, mapValueType: any): number {
        if (this._isNumberValue(valueType)) {
            return this.memberNumberSize(value, valueType);
        } else if (this._isStringValue(valueType)) {
            return this.memberStringSize(value, valueType);
        } else if (value instanceof Array) {
            return this._memberArraySize(value, valueType, arrTypeOrMapKeyType, mapValueType);
        } else if (value instanceof Map) {
            return this._memberMapSize(value, valueType, arrTypeOrMapKeyType, mapValueType);
        } else if (value instanceof BinarySerialize) {
            return value._size();
    ...
    private _memberArraySize(value: any[], valueType: any, arrTypeOrMapKeyType: any, mapValueType: any) {
        //数组的大小
    ...
    private _memberMapSize(value: Map<any, any>, valueType: any, arrTypeOrMapKeyType: any, mapValueType: any) {
        //数组大小
        let typeSize = Uint32Array.BYTES_PER_ELEMENT;
    ...
    private _serialize() {
        ...
        for (let len = serializeKeyList.length, i = 0; i < len; i++) {
            let serializeKey = serializeKeyList[i];
            let [memberName, type, arrTypeOrMapKeyType, mapValueType] = __serialize__[serializeKey];
            this._serializeMember((<any>this)[memberName], memberName, type, arrTypeOrMapKeyType, mapValueType);
        }
    }
     ...
     * @param value 该成员变量的值
     * */
    private _serializeMember(value: any, memberName: string, valueType: any, arrTypeOrMapKeyType: any, mapValueType: any) {
        if (this._isNumberValue(valueType)) {
            this._serializeNumberStreamValue(value, valueType);
        } else if (this._isStringValue(valueType)) {
            this._serializeStringStreamValue(value, valueType, arrTypeOrMapKeyType);
        } else if (value instanceof Array) {
            this._serializeArray(value, memberName, valueType, arrTypeOrMapKeyType, mapValueType);
        } else if (value instanceof Map) {
            this._serializeMap(value, memberName, valueType, arrTypeOrMapKeyType, mapValueType);
        } else if (value instanceof BinarySerialize) {
    ...
    private _serializeNumberStreamValue(value: number, valueType: typeof NumberStreamValue) {
        let type = new valueType();
        type.data = (value === undefined || value === null || value == Number.NaN) ? 0 : value;
        this._byteOffset += type.write(this._dataView, this._byteOffset);
    }

    private _serializeStringStreamValue(value: string, valueType: typeof StringStreamValue, size: number) {
        let type = new valueType();
        type.data = (value === undefined || value === null) ? "" : value;
        this._byteOffset += type.write(this._dataView, this._byteOffset);
    }

    private _serializeArray(value: Array<any>, memberName: string, valueType: any, arrTypeOrMapKeyType: any, mapValueType: any) {
        //先写入数组的大小
        this._dataView.setUint32(this._byteOffset, value.length, Config.USING_LITTLE_ENDIAN);
        this._byteOffset += Uint32Array.BYTES_PER_ELEMENT;
        for (let i = 0; i < value.length; i++) {
            this._serializeMember(value[i], `${memberName}[${i}]`, arrTypeOrMapKeyType, null, null);
        }
    }

    private _serializeMap(value: Map<any, any>, memberName: string, valueType: any, arrTypeOrMapKeyType: any, mapValueType: any) {
        //先写入字典的大小
        this._dataView.setUint32(this._byteOffset, value.size, Config.USING_LITTLE_ENDIAN);
        this._byteOffset += Uint32Array.BYTES_PER_ELEMENT;
        value.forEach((dataValue, dataKey) => {
            ...
            if (arrTypeOrMapKeyType == String) {
                ...
                this._dataView.setUint32(this._byteOffset, dataKey, Config.USING_LITTLE_ENDIAN)
                this._byteOffset += Uint32Array.BYTES_PER_ELEMENT;
            }
            //写值
            this._serializeMember(dataValue, `${memberName}.${dataKey}`, mapValueType, null, null);
        });
    }
    ...
    protected deserialize() {
        ...
        for (let len = serializeKeyList.length, i = 0; i < len; i++) {
            ...
            this._deserializeMember(memberName, type, arrTypeOrMapKeyType, mapValueType);
        }
    }
     ...
     */
    private _deserializeMember(memberName: any, memberType: any, arrTypeOrMapKeyType: any, mapValueType: any) {
        try {
            let originValue = (<any>this)[memberName];
            if (this._isNumberValue(memberType)) {
                (<any>this)[memberName] = this._deserializeNumberStreamValue(memberName, memberType);
            } else if (this._isStringValue(memberType)) {
                (<any>this)[memberName] = this._deserializeStringStreamValue(memberName, memberType, arrTypeOrMapKeyType);
            } else if (originValue instanceof Array) {
                this._deserializeArray(memberName, memberType, arrTypeOrMapKeyType, mapValueType);
            } else if (originValue instanceof Map) {
                this._deserializeMap(memberName, memberType, arrTypeOrMapKeyType, mapValueType);
            } else if (originValue instanceof BinarySerialize) {
    ...
    private _deserializeNumberStreamValue(memberName: any, memberType: typeof NumberStreamValue) {
        let value = new memberType();
        this._byteOffset += value.read(this._dataView, this._byteOffset);
        return value.data;
    }

    private _deserializeStringStreamValue(memberName: any, memberType: typeof StringStreamValue, arrTypeOrMapKeyType: number) {
        let value = new memberType();
        this._byteOffset += value.read(this._dataView, this._byteOffset);
        return value.data;
    }

    private _deserializeArray(memberName: any, memberType: any, arrTypeOrMapKeyType: any, mapValueType: any) {
        //重新解析，初始化时可能已经赋值，需要先清空对象
        (<any>this)[memberName] = [];
        //先读数组大小
        let size = this._dataView.getUint32(this._byteOffset, Config.USING_LITTLE_ENDIAN);
        this._byteOffset += Uint32Array.BYTES_PER_ELEMENT;
        for (let i = 0; i < size; i++) {
            let type = new arrTypeOrMapKeyType();
            if (type instanceof BinarySerialize) {
    ...
    private _deserializeMap(memberName: any, memberType: any, arrTypeOrMapKeyType: any, mapValueType: any) {
        ...
        for (let i = 0; i < size; i++) {
            ...
            if (arrTypeOrMapKeyType == String) {
                ...
                key = this._dataView.getUint32(this._byteOffset, Config.USING_LITTLE_ENDIAN)
                this._byteOffset += Uint32Array.BYTES_PER_ELEMENT;
            }
            //写值
            let data = new mapValueType();
            if (mapValueType instanceof BinarySerialize) {
                data.deserialize();
            } else {
