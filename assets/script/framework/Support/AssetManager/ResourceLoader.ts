...
export default class ResourceLoader {
    ...
    /** @description 加载资源数据 */
    private _resources: Map<string, Resource.ResourceData> = new Map<string, Resource.ResourceData>();
    /**@description 当前已经加载的资源数量 */
    private _loadedCount: number = 0;

    /**@description 加载完成后的数据，为了方便释放时精准释放，没加载成功的资源，不在做释放的判断 */
    private _loadedResource: Map<string, Resource.ResourceInfo> = new Map<string, Resource.ResourceInfo>();
    ...
    /**@description 加载完成回调 */
    private _onLoadComplete: (error: Resource.ResourceLoaderError) => void = null;
    public set onLoadComplete(cb: (error: Resource.ResourceLoaderError) => void) {
    ...
    /**@description 加载进度 */
    public _onLoadProgress: (loadedCount: number, toatl: number, data: Resource.ResourceCacheData) => void;
    public set onLoadProgress(value: (loadedCount: number, toatl: number, data: Resource.ResourceCacheData) => void) {
     ...
     */
    private _getLoadResource: () => Resource.ResourceData[] = null;
    public set getLoadResources(func: () => Resource.ResourceData[]) {
        this._getLoadResource = func;
    }
    public get getLoadResources(): () => Resource.ResourceData[] {
    ...
    public loadResources() {
        ...
        if (!this.getLoadResources) {
            ...
            this.onLoadComplete && this.onLoadComplete(Resource.ResourceLoaderError.NO_FOUND_LOAD_RESOURCE);
            return;
        }
        ...
        if (this._isLoading) {
            ...
            this.onLoadComplete && this.onLoadComplete(Resource.ResourceLoaderError.LOADING);
            return;
        }

        if (this._resources.size > 0 && this.isLoadComplete()) {
            if (CC_DEBUG) cc.warn(`资源已经加载完成，使用已经加载完成的资源`);
            this.onLoadComplete && this.onLoadComplete(Resource.ResourceLoaderError.SUCCESS);
        ...
        this._loadedCount = 0;
        this._resources.forEach((value: Resource.ResourceData, key, source) => {
    ...
    private _unLoadResources() {
        ...
        if (this._resources.size > 0) {
            this._resources.forEach((value: Resource.ResourceData) => {
                if (value.url) {
    ...
    private _onLoadResourceComplete(data: Resource.ResourceCacheData) {
        this._loadedCount++;
