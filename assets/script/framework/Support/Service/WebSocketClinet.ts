...
export class WebSocketClinet {
    public logTag: string = "[WebSocketClient]"
    private _ip: string = "";
    private _port: string = null;
    private _protocol: Socket.WebSocketType = "wss";
    private _dataArr = [];
    ...
    public set connectTimeOut(value: number) { this._conTimeOut = value }
    public get connectTimeOut(): number { return this._conTimeOut }
    /** 发送超时设置 默认为10*/
    private _sendTimeOut: number = 10;
    public set sendTimeOut(value: number) { this._sendTimeOut = value }
    public get sendTimeOut(): number { return this._sendTimeOut }

    private _ws: WebSocket = null;

    private _onOpen: () => void = null;
    public set onOpen(value: () => void) { this._onOpen = value }
    /**@description 网络连接成功 */
    public get onOpen() { return this._onOpen }

    private _onClose: (ev) => void = null;
    public set onClose(value: (ev) => void) { this._onClose = value }
    /**@description 网络关闭 */
    public get onClose() { return this._onClose }

    private _onMessage: (data: Uint8Array) => void = null;
    public set onMessage(value: (data: Uint8Array) => void) { this._onMessage = value }
    /**@description 接收网络数据 */
    public get onMessage() { return this._onMessage }

    private _onError: (ev: Event) => void = null;
    public set onError(value: (ev: Event) => void) { this._onError = value }
    /**@description 网络连接错误 */
    public get onError() { return this._onError }

    private _closeEvent = null;

    private init(ip: string, port: string, protocol: Socket.WebSocketType) {
        this._ip = ip;
        this._port = port;
    ...
    private connectWebSocket(ip: string, port: string, protocol: Socket.WebSocketType) {
        ...
        if (CC_DEBUG) cc.log(this.logTag, `initWebSocket : ${fullUrl}`);
        ...
        //cc.log(this.logTag,`new websocket readyState : ${this._ws.readyState}`);
     ...
     * @param port 端口
     */
    public initWebSocket(ip: string, port: string, protocol: Socket.WebSocketType) {
        if (ip == undefined || ip == null || ip.length < 0) {
            if (CC_DEBUG) cc.error(this.logTag, `init websocket error ip : ${ip} port : ${port}`);
            return;
        }
        //先判断当前是否已经有连接
        if (this._ws) {
            //cc.log(this.logTag,`============initWebSocket111=================`);
            //已经有连接，查看现在的websocket状态
            if (this._ws.readyState == WebSocket.CONNECTING) {
                //当前正在建立连接
                //查看当前连接中的地址是否跟要连接的相同
                if (this._ip == ip && this._port == port) {
                    //cc.warn(this.logTag,"socket正在连接中");
                    return;
                }
                else {
                    if (CC_DEBUG) cc.error(this.logTag, `当前有正在连接的socket??`);
                }
            } else if (this._ws.readyState == WebSocket.OPEN) {
                //当前连接已经打开
                if (this._ip == ip && this._port == port) {
                    if (CC_DEBUG) cc.warn(this.logTag, `当前连接已经是打开的，不重复连接`);
                }
                else {
                    if (CC_DEBUG) cc.error(this.logTag, `当前已经存在连接，请先关闭${this._ip}:${this._port} 再连接 ${ip} : ${port}`);
                }
                ...
                this._ip = ip;
                this._port = port;
                if (CC_DEBUG) cc.warn(this.logTag, `当前网络关闭连接中，关闭完成后重新连接`);
            } else {
                //连接处于关闭状态，直接创建新的连接
                this._ws = null;
    ...
    private __onConected(event) {
        if (this._ws) {
            if (CC_DEBUG) cc.log(this.logTag, `onConected state : ${this._ws.readyState}`)
        }
    ...
    private __onClose(event) {
        ...
        if (event) {
            if (CC_DEBUG) cc.log(this.logTag, `onClose type : ${event.type}`);
        }
        else {
            if (CC_DEBUG) cc.log(this.logTag, `onClose`);
        }

        //等待关闭后连接
        if (this._isWaitingConnect) {
            if (CC_DEBUG) cc.log(this.logTag, `收到连接关闭，有等待连接的网络，重连连接网络`);
            this._closeEvent = null;
    ...
    private __onError(event: Event) {
        if (event) {
            if (CC_DEBUG) cc.error(this.logTag, `onError`, event);
        } else {
            if (CC_DEBUG) cc.error(this.logTag, `onError`);
        }
    ...
    public send(data: string | ArrayBufferLike | Blob | ArrayBufferView) {
        ...
        if (this._ws.readyState === WebSocket.OPEN) {
            ...
            if (this._ws.readyState == WebSocket.CONNECTING) {
                ...
                if (CC_DEBUG) cc.warn(this.logTag, `发送消息失败: ${content}`);
            }
        }
    }
    ...
    public close(isEnd: boolean) {
        ...
        //清空发送
        this._dataArr = [];
        if (CC_DEBUG) cc.log(this.logTag, `close websocket`);
    }
}