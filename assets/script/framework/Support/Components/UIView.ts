...
export class UIView extends EventComponent implements IFullScreenAdapt {
    ...
    /** 是否允许键盘事件 */
    private _isEnableKey: boolean = false

    public get isMaskClose(): boolean { return false }

    public static getPrefabUrl(): string {
        if (CC_DEBUG) { cc.error(`[${cc.js.getClassName(this)}]   请实现public static getPrefabUrl`) }
        return "unknown"
    }

    private _args: Array<any> = null
    public get args(): Array<any> { return this._args }
    ...
    protected set content(value: cc.Node) { this._content = value }
    protected get content() { return this._content }

    /**本组件的类名 */
    private _className: string = "unknow";
    public set className(value: string) { this._className = value }
    public get className(): string { return this._className }

    private _bundle: Resource.BUNDLE_TYPE = null;
    /**指向当前View打开时的bundle */
    public set bundle(value) { this._bundle = value }
    public get bundle() { return this._bundle }

    public close() { Manager.uiManager.close(this.className); }

    /**@description args为open代入的参数 */
    public show(opt: UI.UIShowOption) {
    ...
    isEnabledKeyBack() { return this._isEnableKey }

    onKeyUp(ev: cc.Event.EventKeyboard) {
        if (CC_DEBUG) {
            cc.log(`[${cc.js.getClassName(this)}] onKeyUp keyCode : ${ev.keyCode}`);
        }
        if (ev.keyCode == cc.macro.KEY.escape) { this.onKeyBack(ev) }
