export class BitEncrypt {
    private static readonly logTag = `[BitEncrypt]:`;
    private static _encryptKey: string = "EskKbMvzZBILhcTv";
    public static set encryptKey(value: string) {
        this._encryptKey = value;
    }
    /**@description 加密解密 密钥 */
    public static get encryptKey() {
     ...
     */
    public static decode(content: string, key?: string): string {
     ...
     */
    public static encode(content: string, key?: string): string {
        return this._code(content, key);
    }

    private static _code(content: string, key: string): string {
        ...
        if (result.isOK) {
            ...
            if (CC_DEBUG) cc.error(this.logTag, `encode/decode error content : ${content} key : ${key}`);
            return content;
        }
    }

    private static _check(content: string, key: string): { isOK: boolean, key: string } {
