export class SocketClient {
    private _logtag: string = "SocketClient"
    protected wssCacertUrl: string = null
    ...
    private connectWebSocket(ip: string, port: string, protocol: WebSocketType) {
        ...
        if (CC_DEBUG) cc.log(`[${this._logtag}]`, `initWebSocket : ${fullUrl}`);


        if (CC_JSB && protocol == "wss") {
            if (!this.wssCacertUrl) {
                cc.error(`请先设置wss的证书url,MainController脚本中直接挂载证书`);
            }
            this._ws = new (<any>(WebSocket))(fullUrl, [], this.wssCacertUrl);
        } else {
            this._ws = new WebSocket(fullUrl);
        }
        //cc.log(`[${this._logtag}]`,`new websocket readyState : ${this._ws.readyState}`);
    ...
    public initWebSocket(ip: string, port: string, protocol: WebSocketType) {
        if (ip == undefined || ip == null || ip.length < 0) {
            if (CC_DEBUG) cc.error(`[${this._logtag}]`, `init websocket error ip : ${ip} port : ${port}`);
            return;
        }
        //先判断当前是否已经有连接
        if (this._ws) {
            //cc.log(`[${this._logtag}]`,`============initWebSocket111=================`);
            //已经有连接，查看现在的websocket状态
            if (this._ws.readyState == WebSocket.CONNECTING) {
                //当前正在建立连接
                //查看当前连接中的地址是否跟要连接的相同
                if (this._ip == ip && this._port == port) {
                    //cc.warn(`[${this._logtag}]`,"socket正在连接中");
                    return;
                }
                else {
                    if (CC_DEBUG) cc.error(`[${this._logtag}]`, `当前有正在连接的socket??`);
                }
            } else if (this._ws.readyState == WebSocket.OPEN) {
                //当前连接已经打开
                if (this._ip == ip && this._port == port) {
                    if (CC_DEBUG) cc.warn(`[${this._logtag}]`, `当前连接已经是打开的，不重复连接`);
                }
                else {
                    if (CC_DEBUG) cc.error(`[${this._logtag}]`, `当前已经存在连接，请先关闭${this._ip}:${this._port} 再连接 ${ip} : ${port}`);
                }
                ...
                this._ip = ip;
                this._port = port;
                if (CC_DEBUG) cc.warn(`[${this._logtag}]`, `当前网络关闭连接中，关闭完成后重新连接`);
            } else {
                //连接处于关闭状态，直接创建新的连接
                this._ws = null;
            ...
            //cc.log(`[${this._logtag}]`,`============initWebSocket=================`);
            this.connectWebSocket(ip, port, protocol);
        }
    ...
    private __onConected(event) {
        if (this._ws) {
            if (CC_DEBUG) cc.log(`[${this._logtag}]`, `onConected state : ${this._ws.readyState}`);
        }
    ...
    private __onClose(event) {
        ...
        if (event) {
            if (CC_DEBUG) cc.log(`[${this._logtag}]`, `onClose type : ${event.type}`);
        }
        else {
            if (CC_DEBUG) cc.log(`[${this._logtag}]`, `onClose`);
        }

        //等待关闭后连接
        if (this._isWaitingConnect) {
            if (CC_DEBUG) cc.log(`[${this._logtag}]`, `收到连接关闭，有等待连接的网络，重连连接网络`);
            this._closeEvent = null;
    ...
    private __onError(event: Event) {
        if (event) {
            if (CC_DEBUG) cc.error(`[${this._logtag}]`, `onError`, event);
        } else {
            if (CC_DEBUG) cc.error(`[${this._logtag}]`, `onError`);
        }
    ...
    public send(data: string | ArrayBufferLike | Blob | ArrayBufferView) {
        ...
        if (this._ws.readyState === WebSocket.OPEN) {
            ...
            if (this._ws.readyState == WebSocket.CONNECTING) {
                ...
                if (CC_DEBUG) cc.warn(`[${this._logtag}]`, `发送消息失败: ${content}`);
            }
        }
    }
    ...
    public close(isEnd: boolean) {
        if (this._ws) {
            this._closeEvent = { type: "CustomClose", isEnd: isEnd };
            this._ws.close();
        }
        //清空发送
        this._dataArr = [];
        if (CC_DEBUG) cc.log(`[${this._logtag}]`, `close websocket`);
    }
}