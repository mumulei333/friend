...
export class DefaultCodec extends Codec {
    ...
    Pack(data: MessageStruct): boolean {
        ...
        dataView.setUint32(offset, this.mainCmd, GameConfig.NETWORK_STREAM_USING_LITTLE_ENDIAN);
        offset += Uint32Array.BYTES_PER_ELEMENT;
        dataView.setUint32(offset, this.subCmd, GameConfig.NETWORK_STREAM_USING_LITTLE_ENDIAN);
        offset += Uint32Array.BYTES_PER_ELEMENT;
        dataView.setUint32(offset, this._dataSize, GameConfig.NETWORK_STREAM_USING_LITTLE_ENDIAN);
        offset += Uint32Array.BYTES_PER_ELEMENT;
        if (data.buffer) {
            //感觉这里的复制数据有点low啊
