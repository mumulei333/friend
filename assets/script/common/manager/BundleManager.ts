import { ViewZOrder } from "../../framework/Config/Config";
import { GameConfig } from "../Config/GameConfig";
import { CommonEvent } from "../Event/CommonEvent";
import { i18n } from "../Language/CommonLanguage";
import DownloadLoading from "../Support/DownloadLoading/DownloadLoading";
import { BundleConfig, HotUpdate, AssetManagerCode, AssetManagerState, DownLoadInfo } from "../Support/HotUpdate/HotUpdate";
...
export class BundleManager {
    ...
    public removeLoadedGamesBundle() {
        ...
        while (i--) {
            if (this.loadedBundle[i] != GameConfig.BUNDLE_HALL) {
            ...
            if (code == AssetManagerCode.NEW_VERSION_FOUND) {
                ...
                if (versionInfo.isNeedPrompt) {
                    Manager.alert.show({
                        ...
                        confirmCb: (isOK) => {
                            if (isOK) {
                                Manager.uiManager.open(DownloadLoading, { zIndex: ViewZOrder.Loading, args: [state, versionInfo.name] });
                            } else {
                                //不更新
                                //直接关闭掉游戏
                                cc.game.end();
